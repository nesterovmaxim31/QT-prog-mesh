...
def retranslet(string1):
    string = list(string1)
    N = 0
    M = 0
    while N + 7 < len(string):
        if string[N:N + 6] == list("title:"):
            string[N:N + 7] = list(f"Вопрос № {M}: ")
            M += 1
        N += 1
        if string[N:N + 5] == list("https"):
            while string[N:N + 4] != list(".jpg"):
                del string[N]
            del string[N:N + 4]
    stri = ""
    for e in string:
...
def tabletka(silka):
    ...
    for d in silka:
        if d in ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]:
            M = False
            B += d
        elif not M and not d in ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]:
            break
    if "spec" in silka:
        return get_answers(int(B), "spec")
    elif "homework" in silka:
        return get_answers(int(B), "homework")


# \\\\\\ 13765087
def get_answers(variant, c_type):
    try:
        allanswers = ""
        ...
        for i in r:
            ...
            data2["answers"][i["test_task"]["id"]] = None
        return retranslet(allanswers)
    except:
        return "Возникла проблема при решении теста, обратитесь к Максу"